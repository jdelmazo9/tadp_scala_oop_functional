...
package object Dibujante {
  def dibujar(ast: Nodo, adapterInicial: TADPDrawingAdapter): TADPDrawingAdapter = {
    ast match {
      // Figuras
      case Cuadrado(topLeft, bottomRight)                         => adapterInicial.rectangle(topLeft.position(), bottomRight.position())
      case Triangulo(firstPoint, secondPoint, thirstPoint)        => adapterInicial.triangle(firstPoint.position(), secondPoint.position(), thirstPoint.position())
      case Circulo(center, radius)                                => adapterInicial.circle(center.position(), radius)

      // Transformaciones
      case Colorete(hijo, color)                                  => dibujar(hijo,adapterInicial.beginColor(color)).end()
      case Escala(hijo, escaladoEnX, escaladoEnY)                 => dibujar(hijo,adapterInicial.beginScale(escaladoEnX,escaladoEnY)).end()
      case Rotacion(hijo, angulo)                                 => dibujar(hijo,adapterInicial.beginRotate(angulo)).end()
      case Traslacion(hijo, desplazamientoX, desplazamientoY)     => dibujar(hijo,adapterInicial.beginTranslate(desplazamientoX, desplazamientoY)).end()
